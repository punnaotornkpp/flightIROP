...
export class OperationList implements OnInit {
  ...
  navigateToCreate(type: string) {
    this.router.navigate(['/apps/operation/create'], {
      queryParams: { type },
    });
  }
  ...
  onEditOperation(op: any) {
    this.router.navigate(['/apps/operation/edit', op.generateNumber]);
  }

  onApprove(op: any) {
    this.router.navigate(['/apps/operation/approved', op.generateNumber]);
  }
}
