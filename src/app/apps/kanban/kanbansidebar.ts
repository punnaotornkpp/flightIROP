...
@Component({
  ...
  template: `<p-drawer
    *ngIf="formValue"
    #sidebar
    [(visible)]="parent.sidebarVisible"
    position="right"
    [baseZIndex]="10000"
    styleClass="!w-full lg:!w-[50rem] overflow-auto"
    [showCloseIcon]="false"
  >
    <ng-template #headless>
      <form (submit)="onSave($event)">
        <div
          class="px-6 py-12 border-b border-surface flex items-center justify-between h-16"
        >
          <p-inplace #inplace>
            <ng-template #display>
              <span
                class="text-surface-900 dark:text-surface-0 font-semibold text-lg"
                pTooltip="Click to edit"
                >{{ formValue.title ? formValue.title : 'Untitled' }}</span
              >
            </ng-template>
          ...
          <div class="flex relative">
            ...
            <div
              id="op"
              class="card z-50 absolute shadow-md !rounded-md border border-gray-100 dark:!border-gray-600 !hidden"
              style="right: 50px; top:45px;"
            >
              <div class="grid grid-cols-12 gap-4 flex-col w-60">
                <div class="col-span-12 flex flex-col">
                  <span
                    class="text-surface-900 dark:text-surface-0 font-semibold block w-full border-b border-surface pb-4 mb-2"
                    >Move to...</span
                  >
                  <span
                    pRipple
        ...
        <div class="grid grid-cols-12 gap-8 pt-8 flex-wrap flex-1 flex-col">
          <div class="col-span-12 field px-8 flex flex-col gap-4">
            <label
              for="start"
              class="block text-surface-900 dark:text-surface-0 font-semibold text-lg"
              >Description</label
            >
            <textarea
              id="description"
          ...
          <div
            *ngIf="formValue.taskList.tasks.length || showTaskContainer"
            class="col-span-12 flex flex-col px-8"
          >
          ...
          >
            ...
            <ul
              *ngIf="formValue.taskList?.tasks?.length !== 0"
              class="list-none p-6 flex flex-col gap-4 bg-surface-50 dark:bg-surface-950 border-surface border rounded-border"
            >
              <li
              ...
              >
                ...
                <span
                  ...
                  [ngClass]="{
                    'text-600 line-through': task.completed,
                    'text-900': !task.completed
                  }"
                >
                  {{ task.text }}
                </span>
          ...
          <div class="col-span-12 flex flex-col field px-8">
            <label
              for="assignees"
              class="block text-surface-900 dark:text-surface-0 font-semibold mb-4 text-lg"
              >Assignees</label
            >
            <p-autocomplete
              [appendTo]="sidebar"
            ...
            >
              <ng-template let-assignee #selecteditem>
                <div
                  class="flex items-center gap-2 border rounded-full dark:!border-gray-600 p-2"
                >
                  <div class="flex gap-2 items-center">
                    <img
                      src="/demo/images/avatar/{{ assignee.image }}"
                      [alt]="assignee.name"
                      class="h-8 w-8 border-2 rounded-full border-surface"
                    />
          ...
          <div class="col-span-12 flex flex-col gap-y-6 px-8 mb-12">
            ...
            <div class="flex items-center">
              <span
                class="w-[3.5rem] h-[3.25rem] rounded-full flex items-center justify-center mr-4 bg-surface-200 dark:bg-surface-800"
              >
              ...
              <textarea
                ...
                (keydown.enter)="onComment($event)"
              ></textarea>
            </div>
            <div
              *ngFor="let comment of formValue.comments"
              class="flex items-center rounded-border"
            >
              <span
                *ngIf="comment.image"
                class="w-[3.5rem] h-[3.25rem] rounded-full flex items-center justify-center"
              >
                <img src="/demo/images/avatar/{{ comment.image }}" />
              </span>
              <span
                *ngIf="!comment.image"
                class="w-[3.5rem] h-[3.25rem] rounded-full flex items-center justify-center bg-surface-200 dark:bg-surface-800"
              >
                <i class="pi pi-user"></i>
              </span>
              <div class="ml-4 w-full">
                <div class="flex justify-between mb-2">
                  <span class="block text-surface-900 dark:text-surface-0">{{
                    comment.name
                  }}</span>
                  <span class="block text-surface-700 dark:text-surface-100"
                    >1 Jun 17:58 pm</span
                  >
                </div>
                <span
                  class="block text-surface-900 dark:text-surface-0 border bg-surface-50 dark:bg-surface-950 border-surface p-2 rounded-border"
                  style="word-break: break-all;"
                  >{{ comment?.text }}</span
                >
              </div>
        ...
        <div
          class="flex w-full justify-end border-t border-surface py-8 px-8 gap-4"
        >
          <button
            pButton
            pRipple
            ...
            class="p-button-outlined p-button-secondary border-surface text-surface-900 dark:text-surface-0 w-12 h-12"
          ></button>
          <button
            pButton
            pRipple
