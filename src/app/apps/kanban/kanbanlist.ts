...
@Component({
  ...
  template: `
        ...
        <ng-template #content let-closeCallback="closeCallback">
          <span class="inline-flex items-center gap-1 h-10">
            <input
              ...
              [value]="list.title"
              [(ngModel)]="list.title"
              class="h-full !rounded-r-none w-[10rem]"
            />
            <button
    ...
    <div class="px-6 mb-4 w-full mt-6 flex">
      <button
        ...
        class="py-4 justify-center font-semibold w-full rounded-border"
        (click)="insertCard()"
      ></button>
    </div>
  `,
  host: {
    class:
      'card md:w-[25rem] overflow-hidden shadow-md !rounded-border flex-shrink-0',
  },
  styles: `
        :host ::ng-deep {
            .p-button-label {
