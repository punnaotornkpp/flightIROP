...
@Component({
  ...
  template: `<div class="flex flex-col h-full">
      <div
        class="flex items-center border-b border-surface-200 dark:border-surface-700 p-4 lg:p-12"
      >
        ...
        <div class="mr-2">
          <span
            class="text-surface-900 dark:text-surface-0 font-semibold block"
            >{{ user.name }}</span
          >
          <span class="text-surface-700 dark:text-surface-100"
            >Last active 1 hour ago</span
          >
        </div>
      ...
      >
        <div *ngFor="let message of user.messages">
          <div
            *ngIf="message.ownerId !== 123"
            class="grid gap-4 grid-nogutter mb-6"
          >
            <div class="mr-4 mt-1">
              <img
                src="/demo/images/avatar/{{ user.image }}"
            ...
            <div class="col mt-4">
              <p
                class="text-surface-900 dark:text-surface-0 font-semibold mb-4"
              >
                {{ user.name }}
              </p>
              <span
          ...
          <div
            *ngIf="message.ownerId === defaultUserId"
            class="grid gap-4 grid-nogutter mb-6"
          >
...
export class ChatBox {
  ...
  setMessage() {
    if (this.user) {
      let filteredMessages = this.user.messages.filter(
        (m: any) => m.ownerId !== this.defaultUserId
      );
