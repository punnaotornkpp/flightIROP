...
@Component({
  ...
  >
    ...
    <span
      class="text-surface-700 dark:text-surface-100 font-semibold ml-auto hidden md:inline"
      >{{ user.lastSeen }}</span
    >
  </div> `,
  host: {
...
export class UserCard {
  ...
  ngOnInit() {
    let filtered = this.user.messages.filter(
      (m: { ownerId: number }) => m.ownerId !== 123
    );
